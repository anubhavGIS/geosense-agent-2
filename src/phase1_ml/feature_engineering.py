...
# WGS 84 / UTM zone 45N — Kolkata's zone. The manual's 32644 is zone 44N (Chennai);
# using it here would distort every distance. Degrees (4326) are never used for distance.
METRIC_CRS = 32645
...
def calc_distance_to_nearest(candidates_gdf, layer_gdf, col_name):
    """Distance in metres from each candidate to the nearest feature of the layer."""
...
def load_hospitals(engine):
    """Hospitals from BOTH POI tables — points as-is, area features as centroids."""
    pts = load_layer("SELECT geometry FROM osm_poi WHERE fclass = 'hospital'", engine)
    areas = load_layer("SELECT geometry FROM osm_poi_area WHERE fclass = 'hospital'", engine)
    areas = areas.to_crs(epsg=METRIC_CRS)          # centroid computed in metres,
    areas['geometry'] = areas.geometry.centroid    # not in degrees
...
def assign_flood_risk(candidates_gdf, engine):
    """Flood risk 0-10 from a flood_zones table when available; else 0 placeholder."""
    try:
        flood = load_layer('SELECT geometry, risk_level FROM flood_zones', engine)
        joined = gpd.sjoin(candidates_gdf, flood, how='left', predicate='within')
        risk_map = {'high': 8, 'medium': 5, 'low': 2}
        return joined['risk_level'].map(risk_map).fillna(0)
    except Exception:
        print('  flood_zones table not found — flood_risk set to 0 (placeholder)')
...
def build_feature_table(candidates_gdf, engine):
    ...
    df['dist_road_m'] = calc_distance_to_nearest(candidates_gdf, roads, 'dist_road_m')

    print('Calculating: distance to nearest hospital ...')
    hospitals = load_hospitals(engine)
    df['dist_hospital_m'] = calc_distance_to_nearest(candidates_gdf, hospitals,
                                                     'dist_hospital_m')
...
if __name__ == '__main__':
    ...
    cands = cands.rename(columns={'location_i': 'location_id'})  # undo dBase truncation
    features = build_feature_table(cands, engine)
